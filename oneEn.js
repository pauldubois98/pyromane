...
function resultats() {
    ...
    disp_cases_cote.innerHTML="side of the forest " + cote;    
    
    if(nb_arbres_debut<=1){
        disp_arbres_debut.innerHTML="tree at the beginning " + nb_arbres_debut;
    } else{
        disp_arbres_debut.innerHTML="tree at the beginning " + nb_arbres_debut;
    }

    if(nb_arbres_brules<=1){
        disp_arbres_brules.innerHTML="burned tree " + nb_arbres_brules;
    } else{
        disp_arbres_brules.innerHTML="burned trees " + nb_arbres_brules;
    }

    if(fires<=1){
        disp_feux_utilises.innerHTML="fire used " + fires;
    } else{
        disp_feux_utilises.innerHTML="fires used " + fires;
    }
    
    /////////////////////////////////////////////////////////////////////////////////////
    disp_cases_total.innerHTML=(cote*cote).toString() + " squares in all";

    if(nb_arbres_fin<=1){
        disp_arbres_fin.innerHTML=nb_arbres_fin + " tree at the end";
    } else{
        disp_arbres_fin.innerHTML=nb_arbres_fin + " trees at the end";
    }

    disp_pourcent_brules.innerHTML=pourcent_brules + " % burned";

    if(brule_par_feu<=1){
        disp_brules_par_feu.innerHTML=brule_par_feu + " burned/fire"
    } else{
        disp_brules_par_feu.innerHTML=brule_par_feu + " burned/fire"
    }
